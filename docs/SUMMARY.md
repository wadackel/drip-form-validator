# Summary

* [Usage](usage/README.md)
    - [Basic](usage/basic.md)
    - [Error manipulate](usage/error-manipulate.md)
    - [Validating Array](usage/array.md)
    - [Inline validation](usage/inline-validation.md)
    - [Asynchronous validation](usage/async-validation.md)
    - [Custom Error Messages](usage/custom-error-messages.md)
    - [Normalize values](usage/normalize.md)
    - [Events](usage/events.md)
    - [Custom Validation Rules](usage/custom-rules.md)
    - [Custom Normalizers](usage/custom-normalizers.md)
    - [l18n](usage/l18n.md)
* [Builtin Rules](rules/README.md)
* [Builtin Normalizers](normalizers/README.md)

---

* [API Document](https://tsuyoshiwada.github.io/drip-form-validator/api/)
* [CHANGELOG](https://github.com/tsuyoshiwada/drip-form-validator/blob/master/CHANGELOG.md)

