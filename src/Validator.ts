...
export type Rule = { [index: string]: InternalRuleParams };
...
export default class Validator extends EventEmitter {
   ...
   */
  getRules(): RuleList {
    const results: RuleList = {};

    forEach(this._rules, (list: Rule, field: string) => {
      forEach(list, (params: InternalRuleParams, ruleName: string) => {
        results[field] = {
          ...(results[field] || {}),
          [ruleName]: params.params,
        };
      });
    });
  ...
  mergeRules(rules: RuleList): void {
    ...
    const results: InternalRuleList = {};
        ...
        if (rule) {
          this._rules[field] = {
            ...
            [ruleName]: {
              ...
              params: rule.mapArgsToParams(params.original, this),
            },
          };
        }
      });
  ...
  private getRuleKeysWith(callback: InternalRuleKeysCallback): string[] {
    ...
    forEach(this._rules, (list: Rule, field: string) => {
      forEach(list, ({ params }: InternalRuleParams, ruleName: string): boolean => {
        const rule = Validator.getRule(ruleName);
  ...
  validate(filter?: string | string[]): boolean {
    ...
    forEach(rules, (fieldRules: Rule, field: string) => {
      ...
      forEach(fieldRules, (params: InternalRuleParams, rule: string) => {
        const result = this.syncExecuteTest(rule, field, value, params.params);

        if (result === true) {
          this.removeError(field, rule);
        } else {
          this.addError(field, rule, result, params.params);
        }
      });
    });
  ...
  asyncValidate(filter?: string | string[]): Promise<Values> {
    ...
    return Promise.all(map(rules, (fieldRules: Rule, field: string) => {
      ...
      return Promise.all(map(fieldRules, (params: InternalRuleParams, rule: string) =>
        this.asyncExecuteTest(rule, field, value, params.params),
      ))
