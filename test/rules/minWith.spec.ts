...
const ruleName = 'minWith';

describe(`Rules#${ruleName}`, () => {
  it('Should be return true', () => {
    const v = new Validator({ k1: 11, k2: 10 }, { k1: { [ruleName]: 'k2' } });
    assert(v.validate());
  ...
  it('Should be return false', () => {
    ...
    v.setValues({ k1: 10, k2: 11 });
    assert(v.validate() === false);

    v.setValues({ k1: 10, k2: '11' });
    assert(v.validate() === false);

    v.setValues({ k1: 'str', k2: 4 });
    assert(v.validate() === false);

    v.setValues({ k1: '0.2', k2: 0.3 });
    assert(v.validate() === false);

    v.setValues({ k1: '0.2', k2: '0.3' });
    assert(v.validate() === false);

    v.setValues({ k1: [0, 1, 2], k2: 4 });
    assert(v.validate() === false);
